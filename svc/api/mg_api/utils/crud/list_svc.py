...
class ListSvc(Generic[R, M, LP]):
    ...
    async def __call__(self, params: LP) -> ListSlice[R]:
        ...
        if params.search:
            stmt = await self._apply_search(stmt, params.search, self._search_fields())
    ...
    async def _apply_order(self, stmt, params):
        if attr := getattr(self._M, "created_at", None):
            stmt = stmt.order_by(desc(attr))
        return stmt

    async def _apply_filters(self, stmt, params):
        filters = QueryUtils.parse_filters(self._M, params)
        return stmt.filter(*filters)

    async def _apply_search(self, stmt, search, fields):
        return QueryUtils.apply_search(stmt, search, fields)
